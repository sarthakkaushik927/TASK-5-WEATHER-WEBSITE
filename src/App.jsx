...
import { Sun, Cloud, Wind, Droplets, Search, LogOut, LoaderCircle, Mail, Lock, AlertCircle } from 'lucide-react';
        ...
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <motion.button 
                    ...
                    whileTap={{ scale: 0.98 }}
                    type="submit" 
                    className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-lg transition-colors duration-300 flex items-center justify-center"
                >
                    Login
                </motion.button>
            </form>
...
const WeatherDashboard = ({ onLogout }) => {
    ...
    return (
        ...
        >
            ...
            {error && <motion.div variants={itemVariants} className="bg-red-500/20 border border-red-500 text-red-300 px-4 py-3 rounded-lg text-center mb-6">{error}</motion.div>}
                ...
                <main className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    >
                        ...
                        <div className="flex items-center mt-4 sm:mt-0">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-gray-300 text-center">{weatherData.current.condition.text}</p>
                            </div>
                        </div>
                    </motion.section>
                    ...
                    <motion.section variants={itemVariants} className="lg:col-span-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.98 }} className="bg-[#1E213A]/60 p-4 rounded-xl flex flex-col items-center justify-center text-center">
                                <Wind className="text-gray-400 mb-2" />
                                <h3 className="text-gray-400">Wind Status</h3>
                                <p className="text-3xl font-bold mt-2">{weatherData.current.wind_kph} <span className="text-xl">km/h</span></p>
                            </motion.div>
                             <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.98 }} className="bg-[#1E213A]/60 p-4 rounded-xl flex flex-col items-center justify-center text-center">
                                <Droplets className="text-gray-400 mb-2" />
                                <h3 className="text-gray-400">Humidity</h3>
                                <p className="text-3xl font-bold mt-2">{weatherData.current.humidity} <span className="text-xl">%</span></p>
                            </motion.div>
                            <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.98 }} className="bg-[#1E213A]/60 p-4 rounded-xl flex flex-col items-center justify-center text-center">
                                <Sun className="text-gray-400 mb-2" />
                                <h3 className="text-gray-400">UV Index</h3>
                                <p className="text-3xl font-bold mt-2">{weatherData.current.uv}</p>
                            </motion.div>
                            <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.98 }} className="bg-[#1E213A]/60 p-4 rounded-xl flex flex-col items-center justify-center text-center">
                                <Cloud className="text-gray-400 mb-2" />
                                <h3 className="text-gray-400">Feels Like</h3>
                                <p className="text-3xl font-bold mt-2">{Math.round(weatherData.current.feelslike_c)}°c</p>
                            </motion.div>
                        </div>
                    </motion.section>

                    <motion.section variants={itemVariants} className="lg:col-span-2 bg-[#1E213A]/60 backdrop-blur-md border border-white/10 p-6 rounded-2xl">
                    ...
                    <motion.section variants={itemVariants} className="lg:col-span-1 bg-[#1E213A]/60 backdrop-blur-md border border-white/10 p-6 rounded-2xl">
                         <h2 className="text-2xl font-semibold mb-4">3-Day Forecast</h2>
                         <div className="space-y-3">
                            {weatherData.forecast.forecastday.map(day => (
                                <motion.div 
                                    key={day.date_epoch} 
                                    className="flex justify-between items-center bg-white/10 p-3 rounded-lg cursor-pointer"
